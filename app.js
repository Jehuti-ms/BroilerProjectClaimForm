...
// Calculate where table header should stick (below main header)
function calculateTableHeaderPosition() {
    console.log('Calculating table header position...');
    
    const mainHeader = document.querySelector('.header');
    const tableHeader = document.querySelector('#time-table thead');
    
    if (!mainHeader || !tableHeader) return;
    
    // Get main header height
    const headerHeight = mainHeader.offsetHeight;
    const headerRect = mainHeader.getBoundingClientRect();
    
    // Also account for any margin/padding
    const headerStyle = window.getComputedStyle(mainHeader);
    const marginBottom = parseInt(headerStyle.marginBottom) || 0;
    
    // Calculate total space the header takes up
    const totalHeaderSpace = headerRect.bottom - headerRect.top + marginBottom;
    
    console.log('Header height:', headerHeight, 'px');
    console.log('Header takes up:', totalHeaderSpace, 'px from top');
    
    // Set table header to stick right below the main header
    const stickyPosition = totalHeaderSpace + 5; // +5px for a small gap
    
    // Apply to both thead and th elements
    tableHeader.style.top = stickyPosition + 'px';
    
    // Also update individual th elements
    const thElements = tableHeader.querySelectorAll('th');
    thElements.forEach(th => {
        th.style.top = stickyPosition + 'px';
    });
    
    console.log('Table header will stick at:', stickyPosition, 'px from top');
...
window.addEventListener('load', function() {
    setTimeout(calculateTableHeaderPosition, 100);
});

window.addEventListener('resize', calculateTableHeaderPosition);

// Also run when content changes
document.addEventListener('DOMContentLoaded', function() {
    // Recalculate after any DOM changes
    const observer = new MutationObserver(function() {
        setTimeout(calculateTableHeaderPosition, 50);
    });
